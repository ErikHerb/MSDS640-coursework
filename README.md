...
The project is complete. All five milestones are finished, and the final report, final notebook, and final presentation are in the repo.
...
├── report/           the baseline model report
...
2. Install the packages:
   ...
   ```
3. Make sure `adult.csv` is available to the notebook you are running. The first notebook rebuilds `adult.db` from it, and every notebook after that reads from the database. If the CSV is not present, it can be downloaded from the UCI repository or the Kaggle mirror linked below.
